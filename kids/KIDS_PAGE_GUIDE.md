...
Do not show star ratings, numeric ratings, separate recommendation-reason blocks or multiple sticker tags on event cards. If a detail matters, put it into the highlight summary or facts instead of adding another tag.

The event highlight summary should be one compact paragraph, not two repeated blocks. Chinese starts naturally with `亮点：`; English starts naturally with `Why go:`. It must combine:
...
```text
亮点：Parramatta Square 这场冬季夜游把火盆、沉浸电影、小吃和现场娱乐集中在一起，傍晚去更有节日感，适合全家轻松逛一圈。
```

Good English tone:

```text
Why go: Parramatta Square turns into a winter night-out with firepits, immersive cinema, food and entertainment, making it an easy family outing rather than a kid-only session.
```
