...
- 公式应采用书面工程表达，而不是编程表达：
  - 使用 `≤`、`≥`、`×`、`≈`、`∑`、`√` 等数学符号；不要写成 `<=`、`>=`、`*`、`sqrt()`、`sum()`。
  - 变量和下标要分清，例如 `V` + sub `Ed`，不要写成 `V_Ed`。
  - 避免把很长的英文解释塞进公式行；公式行只放数学关系，解释放在 citation 或 note。
  - 公式中的单位要和数值保持同一行的专业写法，例如 `35 mm`、`48 kN/m`。
  - 如果某个公式只是概念筛查，要在引用行明确写 `Derived relation` 或 `Simplified screen`，不要让读者误以为是规范原公式。
- `formulaTokens` 推荐写法：
  - 普通文字或运算符用 `{ "text": " = " }`。
  - 变量用 `{ "var": "V", "sub": "Ed" }`，需要上标时用 `{ "var": "H", "sup": "2" }`。
  - 希腊字母可直接用 `{ "var": "φ" }`、`{ "var": "γ" }`。
  - 不要用纯字符串拼出 `VEd`、`fck` 这类变量；应通过 `var/sub/sup` 分层渲染。
